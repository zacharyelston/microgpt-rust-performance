...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "-s" | "--steps" => {
                i += 1;
                if i < args.len() {
                    cfg.steps = args[i].parse().unwrap_or(cfg.steps);
                }
            }
            "-l" | "--lr" => {
                i += 1;
            ...
            "-e" | "--emb" => {
                i += 1;
                if i < args.len() {
                    cfg.n_emb = args[i].parse().unwrap_or(cfg.n_emb);
                }
            }
            "-h" | "--head" => {
                i += 1;
                if i < args.len() {
                    cfg.n_head = args[i].parse().unwrap_or(cfg.n_head);
                }
            }
            "-y" | "--layer" => {
                i += 1;
                if i < args.len() {
                    cfg.n_layer = args[i].parse().unwrap_or(cfg.n_layer);
                }
            }
            "-c" | "--ctx" => {
                i += 1;
                if i < args.len() {
                    cfg.n_ctx = args[i].parse().unwrap_or(cfg.n_ctx);
                }
            }
            "-f" | "--ff" => {
                i += 1;
                if i < args.len() {
                    cfg.n_ff_exp = args[i].parse().unwrap_or(cfg.n_ff_exp);
                }
            }
            "--silent" => {
                silent = true;
    ...
    if std::fs::metadata(&cfg.input_file).is_err() {
        let _ = std::process::Command::new("curl")
            .args(["-o", &cfg.input_file, INPUT_URL])
            .output();
    }
